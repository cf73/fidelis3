---
id: 2df3ca22-8c4d-4dc4-ae88-aa5b161b8b37
blueprint: product
manufacturer: cad1d45b-6c89-4812-a990-fdab9e473e07
product_hero_image: boulder-amplifiers-inc-866-integrated-amplifier-an.jpg
available_for_demo: true
quote: 'Not only does it sound better, but it''s more powerful, stylish and affordable than its predecessor, and with the option to add a DAC, this is a real winner.'
quote_attribution: 'What Hi-Fi?'
description:
  -
    type: paragraph
    content:
      -
        type: text
        text: "How do you follow up one of the most successful products in the\_"
      -
        type: text
        text: h
        marks:
          -
            type: link
            attrs:
              href: 'http://boulderamp.com/about-boulder-amplifiers/'
      -
        type: text
        text: 'istory of a company? With a product that improves upon the original in every way, and the Boulder 866 Integrated does just that.'
  -
    type: paragraph
    content:
      -
        type: text
        text: 'Power is up to 200W per channel. Manufacturing is substantially more efficient. Weight has been reduced. New circuitry gives you greater current output at maximum power. External IP control has been added. There are versions for traditional systems as well as modern streaming media users. And a dedicated Boulder app easily enables you to manage everything.'
  -
    type: paragraph
    content:
      -
        type: text
        text: "And here’s the best part: the sound is everything you would expect from a Boulder. It’s powerful, clean, clear, quick, and true to the sound of the original recording and the intents of the artists involved. In fact, there’s absolutely no reason\_"
      -
        type: text
        text: not
        marks:
          -
            type: italic
      -
        type: text
        text: "\_to take home an 866. It’s everything you could want and more."
price: 15500
show_price: true
available_to_buy_online: true
specs:
  -
    type: bullet_list
    content:
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'INPUTS: 3 pairs balanced analog via 3-pin XLR'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'DIGITAL INPUTS: Ethernet, USB x 4, AES3 (adaptable to S/PDIF), Toslink (optical), WiFi'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'OUTPUTS: 0.25" (6 mm) binding posts'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'CONTINUOUS POWER, 8 OHMS: 200W'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'PEAK POWER, 8 OHMS: 250W'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'CONTINUOUS POWER, 4 OHMS: 400W'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'PEAK POWER, 4 OHMS: 400W'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'PEAK POWER, 2 OHMS: 700W'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'DISTORTION, THD: 0.01%'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'EQUIVALENT INPUT NOISE (EIN), 20 KHZ: 2 μV'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'INPUT IMPEDANCE: 100k ohms, balanced'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'MAXIMUM ANALOG GAIN: 40.4 dB'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'FREQUENCY RESPONSE, 20-20 KHZ: 0.00, -0.04 dB'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'FREQUENCY RESPONSE, -3 DB: 0.015 Hz, 150 kHz'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'POWER REQUIREMENTS: 100V, 120V, 240V, 50-60 Hz'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'POWER CONSUMPTION: 1000W maximum'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'DIMENSIONS: 17" W x 15.4 D x 7.25" H'
              -
                type: hard_break
              -
                type: text
                text: '(44 cm W x 39 cm D x 19 cm H)'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'WEIGHT54 lbs.'
              -
                type: hard_break
              -
                type: text
                text: '(24.5 kg)'
      -
        type: list_item
        content:
          -
            type: paragraph
            content:
              -
                type: text
                text: 'SHIPPING DIMENSIONS24" W x 23" D x 14" H'
              -
                type: hard_break
              -
                type: text
                text: '(61 cm H x 59 cm D x 36 cm H)'
reivews_set:
  -
    type: Review
    excerpt:
      -
        type: paragraph
        content:
          -
            type: text
            text: 'The sound. What can I tell you about the sound? It’s not great. By which I mean that it’s excellent… extraordinarily good, in fact. So exceptionally good that you won’t have to spend hours in lengthy listening sessions, auditioning track after track after track in order to establish its quality: you’ll know in the first few instants that you’re listening to a remarkable amplifier. For starters, the 866’s control over bass is exceptional. If you thought your speakers delivered tight, taut and rhythmic bass, you won’t really have heard them until you’ve connected them to Boulder’s 866. The tonal quality, the depth of the bass, the perfect pacing of bass lines, the dynamism revealed by the sound from a close-miked drum kit… the Boulder 866 has it all in spades. No, it’s better than just spades, it’s a full lay-down misère.'
    attribution: 'What Hi-Fi?'
    link: 'https://www.whathifi.com/us/reviews/boulder-866-integrated'
    date_of_review: '2020-05-06'
    enabled: true
title: '866 (Analog)'
product-categories: integrated-amplifiers
system_category: amplification
local_only: false
shipping: 200
updated_by: 87ca4130-78f3-4ed1-8b64-aa552d3d08a8
updated_at: 1681303070
---
