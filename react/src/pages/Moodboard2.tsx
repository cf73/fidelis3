...
export const Moodboard2: React.FC = () => {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        setProducts(productsData.slice(0, 8)); // Show first 8 products for moodboard
      } catch (error) {
  ...
  const styleVariants = {
    ...
    midcentury: {
      card: 'bg-white border border-stone-300 rounded-none shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2',
      title: 'text-lg font-light text-stone-800 mb-2 tracking-wide',
      price: 'text-2xl font-light text-stone-700',
      manufacturer: 'text-sm font-light text-stone-600 tracking-wide',
      category: 'inline-flex items-center px-3 py-1 rounded-none text-xs font-light bg-stone-200 text-stone-700 tracking-wide',
      featured: 'inline-flex items-center px-3 py-1 rounded-none text-xs font-light bg-stone-600 text-stone-100 tracking-wide'
    }
  };
  ...
  return (
    <div className={`min-h-screen ${selectedStyle === 'midcentury' ? 'bg-white' : 'bg-gray-50'}`}>
      {/* Navigation */}
      <nav className={`px-8 py-6 ${selectedStyle === 'midcentury' ? 'bg-white' : 'bg-gray-900'}`}>
        <div className="flex justify-between items-center">
          <div className="flex space-x-8">
            <a href="#" className={`font-light tracking-wide ${selectedStyle === 'midcentury' ? 'text-stone-800' : 'text-white'}`}>PROJECT +</a>
            <a href="#" className={`font-light tracking-wide ${selectedStyle === 'midcentury' ? 'text-stone-800' : 'text-white'}`}>ABOUT +</a>
          </div>
          <div className="flex items-center space-x-4">
            <span className={`font-light ${selectedStyle === 'midcentury' ? 'text-stone-800' : 'text-white'}`}>Cart (2)</span>
          </div>
        </div>
      </nav>

      {/* Hero Section with Musical Hero Image */}
      <div className="relative min-h-screen">
        {/* Background Image - Jimi Hendrix */}
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-8">
                <div className="text-center">
                  <div className={`text-5xl font-light mb-2 ${selectedStyle === 'midcentury' ? 'text-stone-50' : 'text-white'}`}>920+</div>
                  <div className={`text-sm font-light tracking-wide ${selectedStyle === 'midcentury' ? 'text-stone-300' : 'text-gray-400'}`}>Studio Systems</div>
                </div>
                <div className="text-center">
                  <div className={`text-5xl font-light mb-2 ${selectedStyle === 'midcentury' ? 'text-stone-50' : 'text-white'}`}>20k+</div>
                  <div className={`text-sm font-light tracking-wide ${selectedStyle === 'midcentury' ? 'text-stone-300' : 'text-gray-400'}`}>Happy Customers</div>
                </div>
                <div className="text-center">
                  <div className={`text-5xl font-light mb-2 ${selectedStyle === 'midcentury' ? 'text-stone-50' : 'text-white'}`}>90%</div>
                  <div className={`text-sm font-light tracking-wide ${selectedStyle === 'midcentury' ? 'text-stone-300' : 'text-gray-400'}`}>Improvement</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div>
              <div className="space-y-6">
                {['01 Modern Audio System', '02 Luxury Home Theater', '03 Studio Monitoring', '04 Portable Setup'].map((project, index) => (
            ...
            <div className="space-y-8">
              ...
              <div className="relative">
                ...
                <div className={`absolute bottom-4 left-4 p-4 ${selectedStyle === 'midcentury' ? 'bg-stone-50 text-stone-800' : 'bg-white text-black'}`}>
                  <p className="text-sm font-light">Comfortable chair</p>
                  <p className="text-xs font-light">INDOOR</p>
                  <a href="#" className="text-xs font-light underline">Suvav package →</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className={`py-20 ${selectedStyle === 'midcentury' ? 'bg-stone-800' : 'bg-black'}`}>
        <div className="container-custom">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {['Living Room', 'Bedroom', 'Storage & Shelves', 'Dining Room', 'Office Room', 'Kitchen'].map((category, index) => (
      ...
      <div className={`py-20 ${selectedStyle === 'midcentury' ? 'bg-stone-50' : 'bg-gray-50'}`}>
        <div className="container-custom">
          <H2 className={`mb-12 ${selectedStyle === 'midcentury' ? 'text-stone-800 font-light tracking-wide text-4xl' : 'text-gray-900'}`}>
            building your dream system since 1998
          </H2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.slice(0, 6).map((product) => (
              <div key={product.id} className={currentStyle.card}>
                {/* Product Image */}
                <div className="aspect-w-16 aspect-h-9 w-full">
                  {product.product_hero_image ? (
                    <img
                      src={getImageUrl(product.product_hero_image)}
                      alt={product.title}
                      className={`w-full h-48 object-cover ${selectedStyle === 'midcentury' ? 'rounded-none' : 'rounded-t-lg'}`}
                    />
                  ) : (
                    <div className={`w-full h-48 flex items-center justify-center ${selectedStyle === 'midcentury' ? 'bg-stone-300 rounded-none' : 'bg-gray-200 rounded-t-lg'}`}>
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <span className={`text-xs font-light tracking-wide ${selectedStyle === 'midcentury' ? 'text-stone-500' : 'text-gray-500'}`}>
                      3 variants
                    </span>
                    <div className="flex space-x-1">
                      <div className={`w-3 h-3 rounded-full ${selectedStyle === 'midcentury' ? 'bg-stone-400' : 'bg-gray-400'}`}></div>
                      <div className={`w-3 h-3 rounded-full ${selectedStyle === 'midcentury' ? 'bg-stone-600' : 'bg-gray-600'}`}></div>
                      <div className={`w-3 h-3 rounded-full ${selectedStyle === 'midcentury' ? 'bg-stone-800' : 'bg-gray-800'}`}></div>
                    </div>
                  </div>
                </div>
              </div>
